...
void TreeReader::InitContent( TTree* treePtr )
{
    if ( treePtr )
    {
        fTree = treePtr;
...
void TreeReader::FindLeaf(const char* bname)
{
   ...
   // analyze content type of this leaf.
   if (leaf->IsA() == TLeafElement::Class()) {
      std::string descr(leaf->GetBranch()->GetClassName());
...
//______________________________________________________________________________
std::vector<std::string> TreeReader::find_files(const char* patt)
{
