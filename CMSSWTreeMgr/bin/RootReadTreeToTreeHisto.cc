...
int main(int argc, char* argv[])
{
    ...
    try
    { if ( sysPath == "" ) throw "----- error, you need to setup CMSSW envirnment! -----"; }
    catch ( const char* e ) { printf( "%s", e ); }
    ...
    parser.integerValue ("maxEvents"  ) = 100;
    ...
    int maxEvents_          = 100;
    unsigned outputEvery_   = 10000;
    std::string outputFile_ = "reducedTHOutput.root";
    ...
    mainCode.push_back( new root_TreeHistoMain_GGPhoton(&fs) );
    //mainCode.push_back( new root_TreeHistoMain_LbTk(&fs) );
    //mainCode.push_back( new root_TreeHistoMain_GenInfo_LbTk(&fs) );
    //mainCode.push_back( new root_TreeHistoMain_plusminus_LbTk(&fs) );
    //mainCode.push_back( new root_TreeHistoMain_GenInfo_plusminus_LbTk(&fs) );
    ...
    for ( const auto& file : inputFiles_ )
    {
        ...
        //TFile* inFile = TFile::Open( "/home/ltsai/tmp/ggtree_mc_1.root" );
        //inFile->ls();
        if( !inFile ) continue;
