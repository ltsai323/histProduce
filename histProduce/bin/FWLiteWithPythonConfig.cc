...
int main(int argc, char* argv[]) 
{
  ...
  if( !edm::readPSetsFrom(fPath.c_str())->existsAs<edm::ParameterSet>("process") ){
    std::cout << " ERROR: ParametersSet 'process' is missing in your configuration file" << std::endl; exit(0);
  }
  else { std::cout << "able to take PSet\n"; }
  // get the python configuration
  //const edm::ParameterSet& process = edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process");
  const edm::ParameterSet& process = edm::readPSetsFrom(fPath.c_str())->getParameter<edm::ParameterSet>("process");
  const std::vector<edm::ParameterSet> PlotInfo = process.getParameter<std::vector<edm::ParameterSet>>("PlotInfo");
  for ( const auto& info : PlotInfo )
  {
      std::cout << "name = " << info.getParameter<std::string>("HistoName") << std::endl; 
  }
