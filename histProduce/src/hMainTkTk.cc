...
void histMain_TkTk::Process( fwlite::Event* ev )
{
    try 
    {
        ...
        while ( iter != iend )
        {
            ...
            fillHisto( "massTkTk_FakeLam0", twoTk.Mag() );
            // reconstruct k short
            pTk.setMass( pionMass );
            twoTk = pTk + nTk;
            fillHisto( "massTkTk_FakeKshort", twoTk.Mag() );
