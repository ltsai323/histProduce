...
root_TreeHistoMain::root_TreeHistoMain( TFileDirectory* d,  const std::string& pName ) : evtContentReader(), dir(d), preName( pName )
{
    if ( d == nullptr )
    {
...
{
    LOG_INFO("this function %d  && %d", 3, 2);
    if ( NoOutput() ) return;
    unsigned i = 0;
...
bool root_TreeHistoMain::SetInputFile( TFile* inputFile )
{
    ...
    evtContentReader.InitContent(inputTree);
    LoadSourceBranch();
    return true;
}
