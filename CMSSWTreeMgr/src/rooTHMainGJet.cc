...
root_TreeHistoMain_GJet::root_TreeHistoMain_GJet( TFileDirectory* d ) :
    root_TreeHistoMain( d, "GJet" ),
    phoLoadF(),phoLoadI()//, interData()
{
...
root_TreeHistoMain_GJet::~root_TreeHistoMain_GJet()
{}

void root_TreeHistoMain_GJet::Process( unsigned int evtIdx )
{
    // Clear();
    GetEntry( evtIdx );
    ...
    //    phoLoadF.atCand(iCand);
        //phoLoadF.atCand(0);
        //fillHisto( "eta", phoLoadF.var(pF::phoEta) );
        if ( phoLoadF.size() )
        {
            phoLoadF.atCand(0);
            fillHisto( "eta", phoLoadF.var(pF::phoEta) );
...
void root_TreeHistoMain_GJet::GetEntry( unsigned int evtIdx )
{
...
void root_TreeHistoMain_GJet::RegTree()
{
    if ( NoOutput() ) return;
...
void root_TreeHistoMain_GJet::RegHisto()
{
    if ( NoOutput() ) return;
...
void root_TreeHistoMain_GJet::LoadSourceBranch()
{
    TTree* t = readTree();
...
void root_TreeHistoMain_GJet::SummaryCalc()
{
}
