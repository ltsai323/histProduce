#!/usr/bin/env python2
# read enum like format file and make formatTree.cc & .h
# usage :
#    /UpdateFormatTree.py -I ../data/histProduce_MC_fourDaughterI.asc -D ../data/treeContent_MC_fourDaughterD.asc -o MC_fourDaughter --forceWrite
...
import re
templateH = '''
# ifndef __UPDATED_formatTree_{0}__
# define __UPDATED_formatTree_{0}__
# include "histProduce/treeContent/interface/formatTree.h"
struct formatTree_{0} : public formatTree
{{
public:
	enum readVarD
	{{
	...
	formatTree_{0}( int totNumD=comNumD, int totNumI=comNumI ) : formatTree ( totNumD, totNumI ) {{
	}}
	~formatTree_{0}() {{
	}}
}};
...
templateCC = '''
# include "histProduce/treeContent/interface/formatTree{0}.h"
# include <stdio.h>
# include <stdlib.h>

void formatTree_{0}::RegFormatTree(TTree* t)
{{
    if ( t == nullptr )
    {{
...
void formatTree_{0}::LoadFormatSourceBranch(TTree* t)
{{
    if ( t == nullptr )
    {{
...
def formatBranch(varList, type):
    ...
    branchFormat = '''t->Branch( "{0}", &data{1}[{0}], "{0}/{1}" );'''
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Options for create formatTree')
    ...
    # if no filename assigned, use treeFormatTmp.* and use Tmp to be the class name.
    outputFileName = 'Tmp'
    if opt.outName:
        outputFileName = opt.outName
        if os.path.isfile('../interface/formatTree{}.h'.format(outputFileName)):
            if not opt.forceWrite:
                ...
                exit()
    with open('../interface/formatTree{}.h'.format(outputFileName), mode='w') as headerFile:
        enumI = ''
        enumD = ''
    ...
    with open('../src/formatTree{}.cc'.format(outputFileName), mode='w') as ccFile:
        branchD = ''
        branchI = ''
        branchAddD = ''
        branchAddI = ''
    ...
    print '\n\nyour file is created, please check formatTree{0}.h and formatTree{0}.cc'.format(outputFileName)
