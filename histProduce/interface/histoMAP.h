...
public:
    ...
    void createHisto( TFileDirectory* dir, const std::string& name, int nbin, double min, double max );
...
void histoMAP::createHisto( TFileDirectory* dir, const std::string& name, int nbin, double min, double max )
{
    histoMap[name] = dir->make<TH1D>( name.c_str(), name.c_str(), nbin, min, max );
    return;
}
