...
struct plotPrivateSet
{
    ...
    plotPrivateSet ( const std::string& n, const std::string& t, bool f, int lC, int lW, int fC, int fS ) :
        inName ( n ), title ( t ), setFill( f ), lineColor( lC ), lineWidth( lW ), fillColor( fC ), fillStyle( fS ) {}
};
...
int main(int argc, char* argv[])
{
    ...
    for ( const auto& _set : privateSet )
    ...
    TCanvas* cc = new TCanvas( "cc", "cc", 1600, 1000 );
    
    TH1D* h[ pSize ];
    for ( int i=0; i<pSize; ++i )
        h[i] = (TH1D*) inFile->Get( (generalSet.getParameter<std::string>("defaultTree")+"/"+plotSetting[i].inName).c_str() );
