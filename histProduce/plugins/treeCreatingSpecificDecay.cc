...
treeCreatingSpecificDecay::treeCreatingSpecificDecay(const edm::ParameterSet & ps)
{
    ...
    if (useOnia) consume < vector < pat::CompositeCandidate > >(oniaCandsToken, oniaCandsLabel);
    if (usepTks) consume < vector < pat::CompositeCandidate > >(pTksCandsToken, pTksCandsLabel);
    if (usepL0B) consume < vector < pat::CompositeCandidate > >(pL0BCandsToken, pL0BCandsLabel);
    if (usenTks) consume < vector < pat::CompositeCandidate > >(nTksCandsToken, nTksCandsLabel);
    if (usenL0B) consume < vector < pat::CompositeCandidate > >(nL0BCandsToken, nL0BCandsLabel);
    if (useLam0) consume < vector < pat::CompositeCandidate > >(Lam0CandsToken, Lam0CandsLabel);
    if (useLbL0) consume < vector < pat::CompositeCandidate > >(LbL0CandsToken, LbL0CandsLabel);
    if (useLamo) consume < vector < pat::CompositeCandidate > >(LamoCandsToken, LamoCandsLabel);
    if (useLbLo) consume < vector < pat::CompositeCandidate > >(LbLoCandsToken, LbLoCandsLabel);
...
void treeCreatingSpecificDecay::fillDescriptions(edm::ConfigurationDescriptions & descriptions)
{
...
void treeCreatingSpecificDecay::analyze(const edm::Event & ev, const edm::EventSetup & es)
{
    ...
    if (usepL0B)                // Lb->JPsi p K {{{
    {
        ...
        for (unsigned i = 0; i < N; ++i) {
            ...
            const pat::CompositeCandidate & tktkCand =
                *(usefulFuncs::getByRef < pat::CompositeCandidate > (selCand, "refToTkTk"));
            ...
            TLorentzVector fourTkMom(fourTk->x(), fourTk->y(), fourTk->z(),
                                     sqrt(fourTk->x() * fourTk->x() + fourTk->y() * fourTk->y() +
                                          fourTk->z() * fourTk->z() +
                                          selCand.userFloat("fitMass") * selCand.userFloat("fitMass")));
...
void treeCreatingSpecificDecay::fillHisto(const string & name, const pat::CompositeCandidate & cand)
{
...
void treeCreatingSpecificDecay::createHisto(const string & name, int nbin, float hmin, float hmax)
{
    histoMap[name] = fs->make < TH1F > (name.c_str(), name.c_str(), nbin, hmin, hmax);
    return;
}

std::vector < std::pair < std::pair < double, const pat::CompositeCandidate * >, std::pair < double,
    const pat::CompositeCandidate * >>>treeCreatingSpecificDecay::pairpnCands(std::vector < std::pair < double,
                                                                              const pat::CompositeCandidate * >>&pCands,
    ...
    std::vector < std::pair < std::pair < double, const pat::CompositeCandidate * >, std::pair < double,
        const pat::CompositeCandidate * >>>pairpnCandList;
...
bool treeCreatingSpecificDecay::theSameCompCand(const pat::CompositeCandidate * cand1,
                                                const pat::CompositeCandidate * cand2)
{
    if (cand1->numberOfDaughters() != cand2->numberOfDaughters())
        return false;
